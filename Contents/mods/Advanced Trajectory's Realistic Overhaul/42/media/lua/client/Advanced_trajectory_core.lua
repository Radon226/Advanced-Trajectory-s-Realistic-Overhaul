...
aimLevelText:setDefaultColors(1, 1, 1, 0.5)
...
function Advanced_trajectory.determineArrowSpawn(square, isBroken)
    ...
    if isBow then
        ...
        if (isBroken) then
            proj  = instanceItem(proj:getModData().Break)
        end
...
function Advanced_trajectory.searchTargetNearBullet(bulletTable, playerTable, missedShot)
    ...
    local function addTargetsToTable(sq, targetTable, targetSet, player)
        ...
        for zz = 1, movingObjects:size() do
            local mobOrPlayer = movingObjects:get(zz - 1)

            -- Check if the object is an IsoZombie or IsoPlayer
            if instanceof(mobOrPlayer, "IsoZombie") then
                -- check health and zombie dupes
                if mobOrPlayer:getHealth() > 0 and not targetSet[mobOrPlayer] then
                    if isNearWall and not player:CanSee(mobOrPlayer) then 
                        --print('SKIP: IS NEAR WALL AND CANT SEE -- ', player:CanSee(mobOrPlayer))
                    else
                        local entry = { entity = mobOrPlayer, distanceFromPlayer = getTargetDistanceFromPlayer(player, mobOrPlayer), entityType = 'zombie'}
                        table.insert(targetTable, entry)
                        targetSet[mobOrPlayer] = true
                    end
                end
            end
            if instanceof(mobOrPlayer, "IsoAnimal") then
                -- check health and animal dupes
                if mobOrPlayer:getHealth() > 0 and not targetSet[mobOrPlayer] then
                    if isNearWall and not player:CanSee(mobOrPlayer) then 
                        --print('SKIP: IS NEAR WALL AND CANT SEE -- ', player:CanSee(mobOrPlayer))
                    else
                        local entry = { entity = mobOrPlayer, distanceFromPlayer = getTargetDistanceFromPlayer(player, mobOrPlayer), entityType = 'animal'}
                        table.insert(targetTable, entry)
                        targetSet[mobOrPlayer] = true
                    end
                end
            end
            if instanceof(mobOrPlayer, "IsoPlayer") then
                --print("FOUND PLAYER TARGET")
                if allowPVP(player, mobOrPlayer) and not targetSet[mobOrPlayer] then
                    --print("player registered for a meal [it's a bullet]")
                    if isNearWall and not player:CanSee(mobOrPlayer) then 
                        --print('SKIP: IS NEAR WALL AND CANT SEE')
                    else
                        local entry = { entity = mobOrPlayer, distanceFromPlayer = getTargetDistanceFromPlayer(player, mobOrPlayer),  entityType = 'player'}
                        table.insert(targetTable, entry)
                        targetSet[mobOrPlayer] = true
                    end
                end
    ...
    for i, entry in pairs(targetTable) do
        ...
        if not Advanced_trajectory.isObjectFacingZombie(playerTable[1], playerTable[2], bulletTable.dir, targetX, targetY, hitRegThreshold) and not isSteppedOn then
            ...
            elseif targetType == 'animal' then
                --print('Size of animal: ', target:getAnimalSize())
                bulletDistFromTarg = collidedWithTargetHitbox(targetX, targetY, bulletTable, sitHumanoidHitbox) 
            else
...
---------------------------------------------------------
--SPAWN PROJECTILE MODEL AND TEXTURE TO WORLD FUNC SECT--
---------------------------------------------------------
function Advanced_trajectory.addItemToWorld(square, projectileName, x, y, z)
    --print('addItemToWorld -> sqPos: (', square:getX(), ', ', square:getY(), ', ', square:getZ(), ')')
    --print('addItemToWorld -> item: ', projectileName)
    --print('addItemToWorld -> (', x, ', ', y, ', ', z, ')')
    
    -- b42, only method to get type "inventory item"
    local invItem = instanceItem(projectileName)

    square:AddWorldInventoryItem(invItem, advMathFloor(x), advMathFloor(y), advMathFloor(z))
    
    -- Use this to see which square the bullet is in
    --square:Burn()
...
function Advanced_trajectory.boomontick()
    ...
    for indx, tableSfx in pairs(currTable) do
        ...
        if tableSfx.nowSfxNum == 1 and tableSfx.sfxCount == 0 then 
            ...
            local itemOrNone = Advanced_trajectory.addItemToWorld(tableSfx.square, tableSfx.sfxName..tostring(tableSfx.nowSfxNum), tableSfx.pos[1], tableSfx.pos[2], tableSfx.pos[3])
...
function Advanced_trajectory.getAttachmentEffects(weapon)  
    -- HandWeapon:getWeaponpart(String var1) [not case sensitive]
    local scope     = weapon:getWeaponPart('scope')         -- scopes/reddots/sights
    local canon     = weapon:getWeaponPart('canon')         -- britas: bayonets, barrels, chokes
    local stock     = weapon:getWeaponPart('stock')         -- stocks, lasers
    local recoilPad = weapon:getWeaponPart('recoilPad')     -- britas: pad, pistol stock
    local sling     = weapon:getWeaponPart('sling')         -- britas: slings, foregrips, launchers, ammobelts
...
function Advanced_trajectory.OnPlayerUpdate()
    ...
    if isAiming and hasGun and not weaitem:hasTag("Thrown") and not Advanced_trajectory.hasFlameWeapon and not (string.contains(weaitem:getAmmoType() or "", "MandelaArrow")) and not (weaitem:hasTag("XBow") and not getSandboxOptions():getOptionByName("Advanced_trajectory.DebugEnableBow"):getValue()) and (((weaitem:isRanged() and getSandboxOptions():getOptionByName("Advanced_trajectory.Enablerange"):getValue()) or (weaitem:getSwingAnim() =="Throw" and getSandboxOptions():getOptionByName("Advanced_trajectory.Enablethrow"):getValue())) or Advanced_trajectory.FullWeaponTypes[weaitem:getFullType()]) then        
        ...
        -- also returns true if zombie is detected on player level
        if Advanced_trajectory.isAimingOnZombieSteppedOn(player, playerZ, playerNum, mouseX, mouseY, cell) then
            return
        end
        Advanced_trajectory.chooseAimZLevel(player, playerZ, playerNum, mouseX, mouseY, cell)   
...
-- set player aiming down to true if crosshair is on zombie being stepped on
function Advanced_trajectory.isAimingOnZombieSteppedOn(player, playerZ, playerNum, mouseX, mouseY, cell)
    -- Get world coordinates on player's current Z level
    local wx, wy = ISCoordConversion.ToWorld(mouseX, mouseY, playerZ)
    wx, wy = mathfloor(wx), mathfloor(wy)

    -- Get square based off coordinates
    for x = -1, 1 do
        for y = -1, 1 do
            -- Get the grid square at the adjusted position
            local sq = cell:getGridSquare(wx + x + 2.2, wy + y + 2.2, playerZ)

            -- make sure sq is not null
            if sq then
                -- if sq seen on floor Z, return true if zom is detected on that floor and is prone
                if sq:isCanSee(playerNum) then
                    ...
                    for obj = 1, movingObjects:size() do
                        ...
                        -- Check if the object is an IsoZombie/IsoPlayer/IsoAnimal
                        if instanceof(target, "IsoGameCharacter") then
                            Advanced_trajectory.aimlevels = playerZ
                            local distance = Advanced_trajectory.getTargetDistanceFromPlayer(player, target)
                            if target:isProne() and distance < 1 then
...
function Advanced_trajectory.chooseAimZLevel(player, playerZ, playerNum, mouseX, mouseY, cell)
    local minZ = -32
    local maxZ = 32

    if getSandboxOptions():getOptionByName("Advanced_trajectory.enableAutoAimZLevel"):getValue() then
        -- basement level so likely they won't need to kill zombies 2+ floors above or below form basement floor
        if playerZ < 0 then
            minZ = playerZ-1
            maxZ = playerZ+1
        else
            -- local maxIterations = 14
            -- local diffZ = playerZ - maxIterations/2
            -- minZ = playerZ - maxIterations/2
            -- maxZ = playerZ + maxIterations/2
        ...
        local chosenZ = Advanced_trajectory.autoChooseAimZLevel(player, playerZ, playerNum, mouseX, mouseY, minZ, maxZ, cell)
        Advanced_trajectory.aimlevels = chosenZ
    
        -- local str = 'L: ' .. chosenZ  .. ' || ' .. (maxZ - minZ)

        -- aimLevelText:ReadString(UIFont.Medium, str, -1)
        -- aimLevelText:setDefaultColors(1, 1, 1, 0.5)
        -- aimLevelText:AddBatchedDraw(getMouseX() + 35, getMouseY() + 35, true)
    else
        if Advanced_trajectory.manualChooseAimZLevel(player, playerZ, minZ, maxZ) then
            aimLevelText:AddBatchedDraw(getMouseX() + 35, getMouseY() + 35, true)
        end
    end
end
...
-- credits to Lisola
function Advanced_trajectory.manualChooseAimZLevel(player, playerZ, minZ, maxZ)
...
function Advanced_trajectory.autoChooseAimZLevel(player, playerZ, playerNum, mouseX, mouseY, minZ, maxZ, cell)
    ...
    local function loopForTargets(start, stop, num, cell)
        -- Loop through Z levels from minZ to maxZ
        for Z = start, stop, num do
            ...
            for x = -1, 1 do
                for y = -1, 1 do
                    -- Get the grid square at the adjusted position
                    local sq = cell:getGridSquare(wx + x, wy + y, Z)
    ...
    -- player level already checked
    -- priority check: ground level > lower levels > higher levels
    if 0 <= maxZ and 0 >= minZ then
        if loopForTargets(0, 0, 1, cell) then return chosenZ end
    end

    if loopForTargets(playerZ-1, minZ, -1, cell) then return chosenZ end
    if loopForTargets(playerZ+1, maxZ, 1, cell) then return chosenZ end
...
function Advanced_trajectory.dealWithZombieShot(tableProj, tableIndx, zombie, damage, limbShot)
    ...
    elseif not tableProj["nonsfx"]  then
        ...
        if isClient() then
            ...
            Advanced_trajectory.damageZombie(zombie, damage, tableProj.isCrit, limbShot, player)
        end
...
-- singleplayer support atm
function Advanced_trajectory.dealWithAnimalShot(tableProj, animal, damage)
    if not tableProj["nonsfx"]  then
        local player = tableProj.player
        ...
        -- give aim xp upon hit if weapon used is not flamethrower
        if not Advanced_trajectory.hasFlameWeapon then
            Advanced_trajectory.giveOnHitXP(player, animal, damage)
        end

        -- damage animal
        animal:Hit(player:getPrimaryHandItem(), player, damage, false, damage, true)
        if reaction then
            zombie:setHitReaction(reaction)
        end
        ...
        if animal:getHealth() <= 0.1 then                                                 
            if player then
                ...
                if not Advanced_trajectory.hasFlameWeapon then
                    ...
                    triggerEvent("OnWeaponHitCharacter", player, animal, player:getPrimaryHandItem(), damage) 
...
function Advanced_trajectory.dealWithTargetShot(tableProj, tableIndx)
    ...
    ------------------------------------
    ---DEAL WITH ALIVE ANIMAL WHEN HIT--
    ------------------------------------
    if targetType == 'animal' and target:isAlive() then

        -- If zombies are alive, announce the body part it hits if callshot is enabled
        if tableProj.player and getSandboxOptions():getOptionByName("Advanced_trajectory.callshot"):getValue() then
            tableProj.player:Say("Animal hit")
        end

        if Advanced_trajectory.dealWithAnimalShot(tableProj, target, damagezb) then return true end
...
function Advanced_trajectory.updateProjectiles()
    ...
    for indx, tableProj in pairs(currTable) do
        ...
        if squares[1] and squares[#squares] then
            ...
            for i = 1, #squares do 
                if checkCollision(squares[1], tableProj, indx) then
                    ...
                    if  tableProj.weaponName =="Grenade" or tableProj["wallcarmouse"] or tableProj["wallcarzombie"] then
                        --print("updateProjectiles -> blowUp nade from collision")
                        blowUp(tableProj)
                    end
            ...
            tableProj.item = Advanced_trajectory.addItemToWorld(tableProj.square, tableProj.projectileType .. tostring(tableProj.winddir), bulletPosX, bulletPosY, bulletPosZ)
            ...
            ------------------------------
            --UPDATE TRAJECTORY OF NADES--
            ------------------------------
            -- BREAKS GRENADE/THROWABLES 
            if  tableProj["isparabola"]  then
                bulletPosZ = 0.5 - tableProj["isparabola"] * tableProj.currDist * (tableProj.currDist - tableProj.maxRange)
                --print('updateProjectiles -> isparabola: ', tableProj["isparabola"])
                --print('updateProjectiles -> Grenade in parabolic trajectory...zPos <=? 3: ', bulletPosZ)
                if bulletPosZ <= 0.3  then
                    blowUp(tableProj)
                    currTable[indx] = Advanced_trajectory.removeBulletData(tableProj.item) 
                    --print('updateProjectiles -> blowup')
                    break
                end
            end
...
function Advanced_trajectory.OnWeaponSwing(character, handWeapon)
    ...
    local projectilePlayerData = 
    {
        ...
        maxRange = 0,                               --18 weapon max range
    ...
    projectilePlayerData.maxRange =handWeapon:getMaxRange(character)
    ...
    if not ispass then  
        if getSandboxOptions():getOptionByName("Advanced_trajectory.Enablethrow"):getValue() and handWeapon:getSwingAnim() =="Throw" then  --投掷物
            ...
            projectilePlayerData.bulletDist = projectilePlayerData.maxRange
...
-- this will mean projectile will spawn even if gun is empty (it triggers when player swings, including pulling trigger)
--Events.OnWeaponSwing.Add(Advanced_trajectory.OnWeaponSwing)
